...
export default function DashboardAIInsights() {
  ...
  async function deleteInsight() {
    ...
    if (result.isConfirmed) {
      try {
        await fetch("/api/ai/dashboard-insights", { method: "DELETE" })
        setInsight(null)
        setExpanded(false)
      } catch (error) {
      ...
      {!insight ? (
        <div className="px-4 pb-4">
          <p className="text-sm text-muted-foreground mb-3">
            Gere insights inteligentes sobre o seu negocio usando IA
          </p>
          <button
                      ...
                      <div key={i} className="bg-purple-50 dark:bg-purple-950/20 px-2 py-2 rounded">
                        <div className="flex items-center gap-2 mb-1">
                          <span className={`text-[10px] px-1.5 py-0.5 rounded font-medium ${
                            ...
                            a.prioridade === "MEDIA" ? "bg-yellow-500 text-black" :
                            "bg-green-500 text-white"
                          }`}>
                            {a.prioridade}
                          </span>
                          <span className="text-xs font-medium text-foreground">{a.acao}</span>
                        </div>
                        <p className="text-[10px] text-muted-foreground">{a.impacto}</p>
                      </div>
                    ))}
                  </div>
                </div>
