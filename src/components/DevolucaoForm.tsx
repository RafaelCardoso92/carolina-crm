...
// Simplified product type for replacement selection
interface ProdutoForReplacement {
  id: string
  nome: string
  codigo: string | null
  ativo: boolean
...
interface Props {
  ...
  produtos: ProdutoForReplacement[]  // For replacement selection
  onSuccess: () => void
  onCancel: () => void
}
...
export default function DevolucaoForm({
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      {/* Error display */}
      {error && (
        <div className="bg-red-50 dark:bg-red-950/70 text-red-700 dark:text-red-100 p-3 rounded-lg text-sm">
          {error}
        </div>
      )}
