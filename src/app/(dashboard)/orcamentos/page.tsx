...
export default function OrcamentosPage() {
  ...
  async function deleteOrcamento(id: string) {
    if (!confirm("Tem certeza que deseja eliminar este orcamento?")) return
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-medium tracking-wide text-foreground">Orcamentos</h1>
          <p className="text-muted-foreground">{orcamentos.length} orcamentos</p>
        </div>
        <Link
      ...
      <div className="space-y-4">
        {filteredOrcamentos.length === 0 ? (
          ...
          filteredOrcamentos.map(orcamento => (
            <div
              key={orcamento.id}
              className="bg-card rounded-2xl border border-border p-6 hover:border-primary/30 transition"
            >
              ...
              <div className="flex items-center gap-2 mt-4 pt-4 border-t border-border">
                ...
                <button
                  ...
                  className="ml-auto px-4 py-2 text-sm text-red-600 hover:bg-red-50 rounded-lg transition"
                >
                  Eliminar
                </button>
              </div>
